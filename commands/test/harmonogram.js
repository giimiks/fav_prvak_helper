...
module.exports = {
	data: new SlashCommandBuilder().setName('harmonogram').setDescription('Ukáže harmonogram roku a zapíše jej do kalendáře v discordu').setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
		...
		await axios.get(process.env.FAV_API).then((res) => {
			...
			for (let i = 0; i < data.length - 1; i++) {
				if (i < 25) {
					...
					if (interaction.guild.scheduledEvents.cache.find((event) => event.name == data[i].datumOd.value && event.description == data[i].popis) == null) {
